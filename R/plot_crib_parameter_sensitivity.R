# Called by summarise_crib_primary_sensitivity.R after cohort/provenance checks.
export_crib_sensitivity_google_docs <- function(root = ".") {
  root <- normalizePath(root)
  out <- file.path(root, "outputs/final_analysis_crib_adult")
  estimates <- readr::read_csv(
    file.path(out, "parameter_sensitivity/crib_sensitivity_weighted_estimates.csv"),
  ...
  stopifnot(
    ...
    all(estimates$primary_days_md5 == unname(tools::md5sum(
      file.path(out, "datasets/primary_daily_sitting.csv")
    )))
  )
  ...
  stem <- file.path(
    root, "figures/final_analysis_crib_adult/figure5_crib_parameter_sensitivity_google_docs"
  )
...
plot_crib_parameter_sensitivity <- function(changes, frame, cohort, out, root) {
  ...
  readr::write_csv(results,file.path(out,'crib_sensitivity_weighted_estimates.csv'))
  ...
  for(ext in c('png','pdf')) ggplot2::ggsave(file.path(figure_root,paste0('figure5_crib_parameter_sensitivity.',ext)),p,width=14,height=5,dpi=450,bg='white')
