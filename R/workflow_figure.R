...
publication_fig_dir <- file.path(root, "figures", "publication")
google_docs_fig_dir <- file.path(root, "figures", "google_docs")
selected_cutoff_candidates <- c(
  file.path(root, "outputs", "final_analysis", "cutoff", "selected_cutoff.csv"),
  file.path(root, "outputs", "cutoff", "selected_cutoff.csv")
)
selected_cutoff_path <- selected_cutoff_candidates[file.exists(selected_cutoff_candidates)][1]
selected_sleep_cutoff <- 91

if (!is.na(selected_cutoff_path) && file.exists(selected_cutoff_path)) {
...
materialize_source_path <- function(path) {
  if (file.exists(path)) {
    return(path)
  }
  ...
  if (!startsWith(path_norm, paste0(root_norm, "/"))) {
    stop("Missing source file outside project root: ", path)
  }

  rel_path <- substring(path_norm, nchar(root_norm) + 2L)
  temp_path <- tempfile(pattern = "workflow_source_", fileext = paste0("_", basename(path)))
  status <- system2(
    "git",
    c("-C", root, "show", paste0("HEAD:", rel_path)),
    stdout = temp_path,
    stderr = FALSE
  )

  if (!identical(status, 0L) || !file.exists(temp_path) || file.info(temp_path)$size == 0L) {
    unlink(temp_path)
    stop(
      "Missing workflow source file and no tracked git copy was available: ",
      path,
      call. = FALSE
    )
  }

  first_line <- readLines(temp_path, n = 1, warn = FALSE)
  if (length(first_line) == 1L && grepl("^version https://git-lfs.github.com/spec/v1", first_line)) {
    smudged_path <- tempfile(pattern = "workflow_lfs_source_", fileext = paste0("_", basename(path)))
    smudge_status <- system2(
      "git",
      c("-C", root, "lfs", "smudge"),
      stdin = temp_path,
      stdout = smudged_path,
    ...
    if (
      !identical(smudge_status, 0L) ||
        !file.exists(smudged_path) ||
        file.info(smudged_path)$size == 0L
    ) {
      unlink(smudged_path)
      stop(
        "Workflow source file is tracked by Git LFS, but the LFS object is not available locally. ",
        "Run `git lfs fetch --include=\"",
        rel_path,
        "\" --exclude=\"\"` and rerun this script.",
        call. = FALSE
      )
    }

    temp_path <- smudged_path
  }
...
read_raw_day <- function(path) {
  path <- materialize_source_path(path)
...
read_data <- function() {
  ...
  chap <- read_csv(materialize_source_path(chap_path), show_col_types = FALSE) %>%
    mutate(timestamp = as.POSIXct(timestamp, format = "%Y-%m-%d %H:%M:%S", tz = "UTC"))
  swan <- read_csv(materialize_source_path(swan_path), show_col_types = FALSE) %>%
    mutate(
...
hourly_summaries <- function(chap, swan) {
  ...
  sedentary <- chap %>%
    ...
    group_by(hour) %>%
    summarise(percent_sedentary = 100 * mean(tolower(prediction) == "sitting", na.rm = TRUE), .groups = "drop")
  ...
  out %>%
    left_join(sedentary, by = "hour") %>%
    left_join(sleep_nonwear, by = "hour") %>%
    mutate(
      included = percent_sleep_nonwear < selected_sleep_cutoff,
      waking_sedentary = ifelse(included, percent_sedentary, NA_real_)
    )
}
...
make_panel_c <- function(chap, swan, panel_label = "C") {
  ...
  chap_bins <- chap %>%
    mutate(
      ...
      state = if_else(tolower(prediction) == "sitting", "CHAP sedentary", "CHAP non-sedentary")
    ) %>%
    group_by(bin_hour) %>%
  ...
  ggplot(window_df) +
    ...
    breaks = c("CHAP non-sedentary", "CHAP sedentary", "sleep/nonwear", "wake-wear"),
    labels = c(
      "CHAP: non-sedentary",
      "CHAP: sedentary",
      "SWaN: sleep/nonwear",
...
make_panel_d <- function(summary_df, panel_label = "D") {
  ...
  ggplot(summary_df, aes(x = hour)) +
    ...
    geom_line(aes(y = percent_sedentary), color = "#AEB9C8", linewidth = 1.45, na.rm = TRUE) +
    geom_point(aes(y = percent_sedentary), color = "#AEB9C8", size = 3.6, na.rm = TRUE) +
...
final_figure_dir <- Sys.getenv("WORKFLOW_FINAL_FIGURE_DIR", unset = "")

if (nzchar(final_figure_dir)) {
  dir.create(final_figure_dir, showWarnings = FALSE, recursive = TRUE)
  final_png_path <- file.path(final_figure_dir, "fig1_workflow.png")
  final_pdf_path <- file.path(final_figure_dir, "fig1_workflow.pdf")

  ggsave(
    final_png_path,
    plot = fig_three_panel,
    width = 11,
    height = 9.4,
    units = "in",
    dpi = 300,
  ...
  cat("Wrote ", final_png_path, "\n", sep = "")
  cat("Wrote ", final_pdf_path, "\n", sep = "")
} else {
  ...
  dir.create(publication_fig_dir, showWarnings = FALSE, recursive = TRUE)
  dir.create(google_docs_fig_dir, showWarnings = FALSE, recursive = TRUE)
  ...
  fig1_google_docs_png_path <- file.path(google_docs_fig_dir, "fig1.png")
  fig1_publication_png_path <- file.path(publication_fig_dir, "fig1.png")
  fig1_pdf_path <- file.path(publication_fig_dir, "fig1.pdf")
  fig1_tiff_path <- file.path(publication_fig_dir, "fig1.tiff")
  fig1_google_docs <- fig_three_panel & theme(
  ...
  ggsave(fig1_png_path, plot = fig_three_panel, width = 11, height = 9.4, units = "in", dpi = 300, bg = "white")
  ggsave(fig1_google_docs_png_path, plot = fig1_google_docs, width = 11, height = 9.4, units = "in", dpi = round(3000 / 11), type = "cairo", bg = "white")
  ggsave(fig1_publication_png_path, plot = fig_three_panel, width = 11, height = 9.4, units = "in", dpi = 600, bg = "white")
  ggsave(fig1_pdf_path, plot = fig_three_panel, width = 11, height = 9.4, units = "in", bg = "white")
  ggsave(fig1_tiff_path, plot = fig_three_panel, width = 11, height = 9.4, units = "in", dpi = 600, compression = "lzw", bg = "white")
  ...
  cat("Wrote ", fig1_google_docs_png_path, "\n", sep = "")
  cat("Wrote ", fig1_publication_png_path, "\n", sep = "")
  cat("Wrote ", fig1_pdf_path, "\n", sep = "")
  cat("Wrote ", fig1_tiff_path, "\n", sep = "")
}
