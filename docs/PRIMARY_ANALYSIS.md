...
- `R/primary_day_rule.R`: within-day selection and legacy single-day restoration.
- `reports/source/final_analysis_adult.Rmd`: primary estimates, tables, hourly figures, cohort exports, and minimum-two-day sensitivity.
- `R/plot_waking_sedentary_bouts.R`: regenerate the bout figure using exactly `data/derived/primary_daily_sitting.csv`.
- `R/render_primary_report.R`: render the primary HTML locally once source extracts and refreshed bout outputs are available.
- `tests/check_primary_day_rule.R`: selection, single-day restoration, independent-day retention, and hourly coverage checks.
...
primary sample; see [parameter sensitivity documentation](PARAMETER_SENSITIVITY.md).
