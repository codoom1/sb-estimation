...
in `data/derived/primary_daily_sitting.csv`. Non-wear runs bounded by matching or
...
for batch in $(seq 1 50); do
  Rscript --vanilla R/run_parameter_sensitivity.R --batch-index="$batch" --n-batches=50
done
Rscript --vanilla tests/check_paired_sensitivity.R
Rscript --vanilla R/summarise_parameter_sensitivity.R
```
...
Outputs are in `outputs/final_analysis_adult/parameter_sensitivity/`:
...
```sh
rsync -av unity:/work/pi_jstauden_umass_edu/pctSitting_research/outputs/final_analysis_adult/parameter_sensitivity/ outputs/final_analysis_adult/parameter_sensitivity/
pdflatex -interaction=nonstopmode -halt-on-error -output-directory=outputs/final_analysis_adult/parameter_sensitivity outputs/final_analysis_adult/parameter_sensitivity/table_sedentary_differences.tex
```
...
hashes and batch provenance accompany the outputs. Recalculate all settings
with `R/run_parameter_sensitivity.R`.

Validation: `Rscript tests/check_paired_sensitivity.R` checks paired-test
...
The summarizer also calls `R/plot_parameter_sensitivity.R` to estimate
...
Figure 5 is saved as PNG and PDF under `figures/final_analysis_adult/`.
For Google Docs, use `figure5_parameter_sensitivity_google_docs.png`,
...
`figure5_parameter_sensitivity_google_docs.pdf`. The same 36 estimates
are in `parameter_sensitivity/parameter_sensitivity_weighted_estimates.csv`.
